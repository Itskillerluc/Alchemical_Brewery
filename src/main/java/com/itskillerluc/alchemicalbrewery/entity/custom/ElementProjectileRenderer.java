...
public class ElementProjectileRenderer extends EntityRenderer<ElementProjectileEntity> {
    ...
    public static final RenderStateShard.OutputStateShard TRANSLUCENT_BUT_GOOD_OUTPUTSHARD = new RenderStateShard.OutputStateShard("translucent_but_good", () -> {
    ...
    public void render(ElementProjectileEntity pEntity, float pEntityYaw, float pPartialTicks, PoseStack pMatrixStack, MultiBufferSource pBuffer, int pPackedLight) {
        ...
        BakedModel bakedmodel = this.itemRenderer.getModel(pEntity.getElement().itemModel, pEntity.level, null, pEntity.getId());
        this.itemRenderer.renderStatic((LivingEntity) null, pEntity.getElement().itemModel, ItemTransforms.TransformType.FIXED, false, pMatrixStack, pBuffer, pEntity.level, pPackedLight, OverlayTexture.NO_OVERLAY, 1);
